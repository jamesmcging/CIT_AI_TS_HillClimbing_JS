...
class Log {
  ...
  // Name of the state generator used to create the child states
  String sStateGenerator = "";
  ...
  public Log(String sAlgorithmName, int nRunTimes, String sStateGenerator, int nChildStates, String sCityMap) {
    this.sAlgorithmName = sAlgorithmName;
    this.nRunTimes = nRunTimes;
    this.sStateGenerator = sStateGenerator;
  ...
  private RunRecord getBestRun() {
    ...
    Route shortestRoute = bestRun.getShortestRoute();
    
    // Iterate through all the records looking for a shorter route
    for (int i = 0; i < log.size(); i++) {
      // record the shortest run we found
      if (log.get(i).getShortestRoute().getRouteLength() < shortestRoute.getRouteLength()) {
        bestRun = log.get(i);
      }
    }
  ...
  public Route getShortestRoute() {
    // Set the shortest route to be the shortest route in the first run of the
    // algorithm
    Route shortestRoute = log.get(0).getShortestRoute();
    
    // Iterate over the log looking for a shorter route
    for (int i = 0; i < log.size(); i++) {
      if (log.get(i).getShortestRoute().getRouteLength() < shortestRoute.getRouteLength()) {
         shortestRoute = log.get(i).getShortestRoute();
      }
    }
  ...
  void printLogToFile() throws IOException {
    // Get the location of the log file directory
    String sFilePath = new File("").getAbsolutePath()+ "\\src\\logs\\";
    String sFileName = sFilePath + this.nRunTimes + "X" + this.sAlgorithmName + "_" + this.sStateGenerator + "X" + this.nChildStates + "_" + this.sCityMap + ".txt";
